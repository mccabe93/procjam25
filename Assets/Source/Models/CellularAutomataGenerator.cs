...
/// </summary>
public class CellularAutomataLevelGenerator<T>
{
    private readonly CellularAutomataConfiguration<T> _configuration;
    ...
    public CellularAutomataLevelGenerator(CellularAutomataConfiguration<T> configuration)
    {
        ...
        for (int x = 0; x < _configuration.Width; x++)
        {
            for (int y = 0; y < _configuration.Height; y++)
            {
                ...
                if (
                    x == 0
                    || y == 0
                    || x == _configuration.Width - 1
                    || y == _configuration.Width - 1
                    || roll < _configuration.RandomFillFrequency
                )
                {
    ...
    private void PopulateActors(int[,] state)
    {
        for (int x = 1; x < _configuration.Width - 1; x++)
        {
            for (int y = 1; y < _configuration.Height - 1; y++)
            {
                ...
                {
                    double roll = _rng.NextDouble();
                    if (roll < _configuration.ActorFrequency)
                    {
                        state[x, y] = 2;
                    }
                }
